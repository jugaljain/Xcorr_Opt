...
mod = SourceModule("""
	...
	__global__ void stencil_2da(float *in, float *out){
		...
		int idx = threadIdx.y + (threadIdx.x * WIN_SIZE) + (blockIdx.y * WIN_PIX) + (blockIdx.x * NUM_NEIGH * NUM_MAPS) + (threadIdx.z * NUM_PIX * NUM_MAPS * NUM_NEIGH);
		int gidx = (z * NUM_ROWS * NUM_COLS) + (x * NUM_COLS) + y;
	...
	__global__ void corr(float *in1, float *in2, float *ma, float *sda, float *mb, float *sdb, float *out){
		int idx = blockIdx.y + (blockIdx.x * NUM_NEIGH) + (threadIdx.z * NUM_PIX * NUM_NEIGH);
		for(int x = 0; x < WIN_PIX; x++){
			int gidx = x + (blockIdx.y * WIN_PIX) + (blockIdx.x * NUM_NEIGH * 25) + (threadIdx.z * NUM_PIX * NUM_NEIGH * WIN_PIX);
			out[idx] = (in1[gidx] - ma[idx]) * (in2[idx] - mb[idx]) / ((WIN_PIX - 1) * sdb[idx] * sda[idx]);
		}
	}
""")
...
a = numpy.random.randn(NUM_MAPS,NUM_ROWS,NUM_COLS).astype(numpy.float32)	#first feature map
c = numpy.random.randn(NUM_MAPS,(NUM_ROWS+4)*NUM_COLS,WIN_SIZE,WIN_SIZE).astype(numpy.float32)  #unrolled second
ma = numpy.zeros((NUM_MAPS,NUM_ROWS*NUM_COLS,NUM_NEIGH), dtype=numpy.float32)		#mean values
sda = numpy.zeros((NUM_MAPS,NUM_ROWS*NUM_COLS,NUM_NEIGH), dtype=numpy.float32)		#std dev values
aint = numpy.random.randn(NUM_MAPS,NUM_ROWS*NUM_COLS,NUM_NEIGH,WIN_SIZE,WIN_SIZE).astype(numpy.float32)	#unrolled first
caint = numpy.random.randn(NUM_MAPS,NUM_ROWS*NUM_COLS,NUM_NEIGH,WIN_SIZE,WIN_SIZE).astype(numpy.float32)	#further unrolled second
final = numpy.random.randn(NUM_MAPS,NUM_ROWS*NUM_COLS,NUM_NEIGH).astype(numpy.float32)
...
mb_gpu = cuda.mem_alloc(ma.nbytes)
sda_gpu = cuda.mem_alloc(sda.nbytes)
sdb_gpu = cuda.mem_alloc(sda.nbytes)
...
cuda.memcpy_htod(mb_gpu, ma)
cuda.memcpy_htod(sda_gpu, sda)
cuda.memcpy_htod(sdb_gpu, sda)
...
func(b_gpu, c_gpu,grid=((NUM_ROWS+4)*NUM_COLS,1,1),block=(WIN_SIZE,WIN_SIZE,NUM_MAPS))
func2 = mod.get_function("stencil_2da")
func2(a_gpu, aint_gpu, grid=(NUM_ROWS*NUM_COLS,NUM_NEIGH,1), block=(WIN_SIZE,WIN_SIZE,NUM_MAPS))
func3 = mod.get_function("stencil_2dc")
func3(c_gpu, caint_gpu, grid=(NUM_ROWS*NUM_COLS,NUM_NEIGH,1), block=(WIN_SIZE,WIN_SIZE,NUM_MAPS))
func4 = mod.get_function("mean")
func4(caint_gpu, mb_gpu, grid=(NUM_ROWS*NUM_COLS,NUM_NEIGH,1), block=(1,1,NUM_MAPS))
func4(aint_gpu, ma_gpu, grid=(NUM_ROWS*NUM_COLS,NUM_NEIGH,1), block=(1,1,NUM_MAPS))
func5 = mod.get_function("stddev")
func5(caint_gpu, ma_gpu, sdb_gpu, grid=(NUM_ROWS*NUM_COLS,NUM_NEIGH,1), block=(1,1,NUM_MAPS))
func5(aint_gpu, mb_gpu, sda_gpu, grid=(NUM_ROWS*NUM_COLS,NUM_NEIGH,1), block=(1,1,NUM_MAPS))

func6 = mod.get_function("corr")
func6(caint_gpu, aint_gpu, mb_gpu, sdb_gpu, ma_gpu, sda_gpu, final_gpu, grid=(444,60,1), block=(1,1,1))
end = time.time()
